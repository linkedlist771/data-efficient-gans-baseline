...
@click.option(
    "--resolution", help="Resolution of the output images", default=128, type=int
)
@click.option("--num-phases", help="Number of phases", default=5, type=int)
@click.option(
    "--transition-frames",
    help="Number of transition frames per phase",
    default=20,
    type=int,
)
@click.option(
    "--static-frames", help="Number of static frames per phase", default=5, type=int
)
@click.option(
    "--trunc",
    "truncation_psi",
...
def generate_gif(
    ...
    def to_image_grid(outputs):
        ...
        return Image.fromarray(outputs).resize(
            (resolution * num_cols, resolution * num_rows), Image.Resampling.LANCZOS
        )

    def generate(dlatents):
        images = G.synthesis(dlatents, noise_mode=noise_mode)
        images = (
            (images.permute(0, 2, 3, 1) * 127.5 + 128)
            .clamp(0, 255)
            .to(torch.uint8)
            .cpu()
            .numpy()
