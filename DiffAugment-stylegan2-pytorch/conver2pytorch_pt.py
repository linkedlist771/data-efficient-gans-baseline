...
def main():
    parser = argparse.ArgumentParser(
        description="Convert StyleGAN2 network pickle to PyTorch .pt format."
    )
    parser.add_argument(
        "--source_pkl", type=str, help="Path to the source pickle file."
    )
    parser.add_argument(
        "--dest_pt", type=str, help="Path to save the converted .pt file."
    )
    parser.add_argument(
        "--force-fp16", action="store_true", help="Whether to force FP16 precision."
    )
